...
class GalaxyScene: SCNScene {
    ...
    convenience init(create:Bool) {
        ...
        let centerPoint = SCNVector3(0,0,-0.5)
        
//        setupCameraAndLights()
        
        let sun = SCNSphere(radius: 0.1)
        ...
        sunLight.attenuationFalloffExponent = 0.2
        sunLight.attenuationEndDistance = 0.20
//
...
//        // which cat of nodes can be effected by the light
        ...
        sunNode.position = centerPoint

        rootNode.addChildNode(sunNode)

        sunNode.runAction(SCNAction.repeatForever(SCNAction.rotate(by: CGFloat(Double.pi * 0.2), around: SCNVector3(0,0.1,0), duration: 60)))
...
//
        let planetX = SCNSphere(radius: 0.02)
...
//
        ...
        emptyPlanetAttach.pivot = SCNMatrix4MakeTranslation(0, 0, -0.2)
        emptyPlanetAttach.addChildNode(planetXNode)
        
        emptyPlanetAttach.position = centerPoint
//
        rootNode.addChildNode(emptyPlanetAttach)
//
        emptyPlanetAttach.runAction(SCNAction.repeatForever(SCNAction.rotate(by: CGFloat(Double.pi * 2), around: SCNVector3(0.1,0.1,0), duration: 10)))
        planetXNode.runAction(SCNAction.repeatForever(SCNAction.rotate(by: CGFloat(Double.pi * 1.5), around: SCNVector3(0.1,0.1,0), duration: 2)))
//
        let planetXMoon = SCNSphere(radius: 0.005)
        ...
        moonNode.pivot = SCNMatrix4MakeTranslation(0, 0, -0.025)
//
        emptyPlanetAttach.addChildNode(moonNode)
        moonNode.runAction(SCNAction.repeatForever(SCNAction.rotate(by: CGFloat(Double.pi * 1), around: SCNVector3(0,0.1,0), duration: 1)))
//
//
...
//        myParticleSystem.particleSize = 0.0005
//        myParticleSystem.particleSizeVariation = 0.001
...
//        myParticleSystem.emitterShape = SCNSphere(radius: 0.005)
